...
**CRITICAL: Use ABSOLUTE paths** in hook commands. Relative paths like `python .claude/hooks/...` break when the shell cwd changes (e.g., `cd` into a subdirectory with its own `.claude/` folder). This causes a total deadlock where ALL Write/Edit/Bash tools are blocked.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python /absolute/path/to/workspace/.claude/hooks/universal-gate-enforcer.py"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python /absolute/path/to/workspace/.claude/hooks/test-failure-detector.py"
          }
        ]
      }
    ]
